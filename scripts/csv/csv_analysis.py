...
class NBACSVAnalyzer:
    ...
    def validate_with_frictionless(self) -> Dict[str, Any]:
        ...
        try:
            ...
            report_path = self.output_dir / f"{self.csv_name}_frictionless_validation.json"
    ...
    def apply_basketball_validation_rules(self) -> Dict[str, Any]:
    ...
    def _get_validation_rules(self, csv_type: str) -> Dict[str, callable]:
        ...
        if csv_type in ["player_stats", "player_totals", "player_per_game", "per100_possessions", "per36_minutes"]:
            base_rules.update({
        ...
        if csv_type == "draft_history":
            base_rules.update({
                "draft_logic": self._validate_draft_logic
            })
    ...
    def _validate_data_completeness(self) -> Dict[str, Any]:
        ...
        if avg_completeness > 0.95:
            return {"status": "passed", "message": ".1%"}
        elif avg_completeness > 0.80:
            return {"status": "warning", "message": ".1%"}
        else:
            return {"status": "failed", "message": ".1%", "details": completeness_scores}
            ...
            if dtype == pl.Utf8:
                ...
                try:
                    ...
                    if numeric_count > len(self.df) * 0.8:  # 80% look numeric
                        issues.append(f"Column {col} appears numeric but stored as string")
                except:
                    pass

        if not issues:
            return {"status": "passed", "message": "Data types appear appropriate"}
        else:
            return {"status": "warning", "message": f"Potential data type issues: {len(issues)}", "details": issues}

    def _validate_logical_constraints(self) -> Dict[str, Any]:
        """Validate basic logical constraints."""
        issues = []

        # Check for negative values in count statistics
        count_columns = [col for col in self.df.columns if any(term in col.lower() for term in ['pts', 'fgm', 'fga', 'ftm', 'fta', 'reb', 'ast', 'stl', 'blk', 'tov'])]
        ...
        if not issues:
            return {"status": "passed", "message": "No logical constraint violations found"}
        else:
            return {"status": "failed", "message": f"Logical constraint violations: {len(issues)}", "details": issues}

    def _validate_shooting_logic(self) -> Dict[str, Any]:
        ...
        if not issues:
            return {"status": "passed", "message": "Shooting logic validation passed"}
        else:
            return {"status": "failed", "message": f"Shooting logic violations: {len(issues)}", "details": issues}

    def _validate_efficiency_ranges(self) -> Dict[str, Any]:
        ...
        if 'TS_PCT' in self.df.columns:
            ...
            if out_of_range > 0:
                issues.append(f"TS_PCT out of reasonable range [0.2,0.8] in {out_of_range} rows")
        ...
        if not issues:
            return {"status": "passed", "message": "Efficiency ranges validation passed"}
        else:
            return {"status": "failed", "message": f"Efficiency range violations: {len(issues)}", "details": issues}

    def _validate_per_ranges(self) -> Dict[str, Any]:
        ...
        if 'PER' in self.df.columns:
            ...
            if out_of_range > 0:
                issues.append(f"PER out of reasonable range [0,40] in {out_of_range} rows")

        if not issues:
            return {"status": "passed", "message": "PER ranges validation passed"}
        else:
            return {"status": "failed", "message": f"PER range violations: {len(issues)}", "details": issues}
    ...
    def _validate_temporal_consistency(self) -> Dict[str, Any]:
        ...
        return {"status": "passed", "message": "Temporal consistency validation passed"}

    def _validate_score_progression(self) -> Dict[str, Any]:
        """Validate score progression makes sense."""
        # This would validate score changes match event types
        return {"status": "passed", "message": "Score progression validation passed"}

    def _validate_score_calculation(self) -> Dict[str, Any]:
        ...
        if all(col in self.df.columns for col in ['PTS', 'FGM', 'FTM', 'FG3M']):
            ...
            discrepancies = ((self.df['PTS'] - expected_pts).abs() > 1).sum()  # Allow small rounding differences
            if discrepancies > 0:
                issues.append(f"PTS calculation discrepancies in {discrepancies} rows")

        if not issues:
            return {"status": "passed", "message": "Score calculation validation passed"}
        else:
            return {"status": "failed", "message": f"Score calculation violations: {len(issues)}", "details": issues}

    def _validate_game_logic(self) -> Dict[str, Any]:
        ...
        if not issues:
            return {"status": "passed", "message": "Game logic validation passed"}
        else:
            return {"status": "failed", "message": f"Game logic violations: {len(issues)}", "details": issues}

    def _validate_draft_logic(self) -> Dict[str, Any]:
        """Validate draft-specific logic."""
        issues = []

        # Draft picks should be 1-60 (2 rounds of 30)
        if 'OVERALL_PICK' in self.df.columns:
            out_of_range = ((self.df['OVERALL_PICK'] < 1) | (self.df['OVERALL_PICK'] > 60)).sum()
            if out_of_range > 0:
                issues.append(f"OVERALL_PICK out of range [1,60] in {out_of_range} rows")

        if not issues:
            return {"status": "passed", "message": "Draft logic validation passed"}
        else:
            return {"status": "failed", "message": f"Draft logic violations: {len(issues)}", "details": issues}
    ...
    def generate_visualizations(self) -> List[str]:
        ...
        try:
            ...
            numeric_cols = [col for col in self.df.columns if self.df[col].dtype in [pl.Int64, pl.Float64]]
    ...
    def calculate_quality_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "duplicate_percentage": (self.df.is_duplicated().sum() / len(self.df)) * 100,
        ...
        if len(self.df) > 0:
            metrics["uniqueness_score"] = (1 - metrics["duplicate_percentage"] / 100) * 100

        # Validity score based on validation results
        if self.validation_results:
            total_validations = len(self.validation_results.get("passed", [])) + len(self.validation_results.get("failed", []))
            if total_validations > 0:
                passed_validations = len(self.validation_results.get("passed", []))
                metrics["validity_score"] = (passed_validations / total_validations) * 100
    ...
    def generate_summary_report(self) -> str:
        ...
        summary = {
            ...
            "profile_summary": {
                ...
                "missing_data_percentage": profile_data.get("overview", {}).get("missing_cells_percentage", 0),
...
def main():
    ...
    parser.add_argument("--csv-dir", default="../csv_files", help="Directory containing CSV files")
    parser.add_argument("--all-csvs", action="store_true", help="Analyze all CSV files in csv-dir")
