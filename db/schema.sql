...
-- Assumption: This file is executed on a fresh/empty PostgreSQL database (no DROP TABLE statements).
-- Schema: public
-- Encoding: UTF8
-- Notes:
-- - Types and nullability chosen to align with docs/phase_0_csv_inventory.json and Phase 1 rules.
-- - No constraints are added that would contradict observed nullability in the CSV inventory.
-- - Surrogate keys are integer/bigint where appropriate; natural keys are preserved as unique where useful.
...
-- BRIN for chronological queries
